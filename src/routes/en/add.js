...
router.get('/panel/add', auth.isLoggedIn_en, async (req, res) => {
    if (req.query.message) {
        res.render('pages/en/add', {stylesheet: '/css/pages/add.css', message: req.query.message});
    } else {
        res.render('pages/en/add', {stylesheet: '/css/pages/add.css'});
    }
});
        ...
        if (row[0].ping == linkedSend.ping) {
            ...
            if (!(row.length > 0)) {
                if ((linkedSend.name.length > 0) && (linkedSend.password.length > 0)) {
                    ...
                    res.redirect('/en/panel/add?message=' + message);
                }
            } else {
