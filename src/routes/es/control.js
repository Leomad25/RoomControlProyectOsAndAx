...
router.get('/panel/control', auth.isLoggedIn_en, async (req, res) => {
    ...
    if (check.length > 0) {
        ...
        if (req.query.message) {
            ...
            res.render('pages/es/control', {stylesheet: '/css/pages/control.css', script: '/js/pages/control.js', deviceOptions, iddevice, message});
        } else {
            res.render('pages/es/control', {stylesheet: '/css/pages/control.css', script: '/js/pages/control.js', deviceOptions, iddevice});
        }
    } else {
        message = 'You+do+not+have+access+to+this+device.';
        res.redirect('/es/panel?message=' + message);
    }
});
...
router.post('/panel/control', auth.isLoggedIn_en, async (req, res) => {
    ...
    if (check.length > 0) {
        ...
        res.redirect('/es/panel/control?iddevice=' + send.iddevice);
    }
});
...
router.post('/panel/control/changeNameOption', auth.isLoggedIn_en, async (req, res) => {
    ...
    if (check.length > 0) {
        if (req.body.valueField.length > 0) {
            ...
            res.redirect('/es/panel/control?iddevice=' + req.query.iddevice);
        } else {
            message = 'The+name+is+invalid.';
            res.redirect('/es/panel/control?iddevice=' + req.query.iddevice + '&message=' + message);
        }
    } else {
        res.redirect('/es/panel/panel');
    }
});
